...
def write_config_file_cb(b):
    dirname = os.path.expanduser('~/.local/share/motility')
...
def get_config_files():
    ...
    if nanoHUB_flag:
        full_path = os.path.expanduser("~/data/results/.submit_cache/motility")
    else:
        # local cache
        try:
            cachedir = os.environ['CACHEDIR']
            full_path = os.path.join(cachedir, "motility")
        except:
...
def run_done_func(s, rdir):
    ...
    if nanoHUB_flag:
        ...
        os.system("submit  mail2self -s 'nanoHUB motility' -t 'Your Run completed.'&")
...
def run_sim_func(s):
    ...
    if nanoHUB_flag:
        if remote_cb.value:
            s.run(run_name, "-v ncn-hub_M@brown -n 8 -w 1440 motility-r7 config.xml")   # "-r7" suffix??
        else:
